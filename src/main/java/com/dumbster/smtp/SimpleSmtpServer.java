...
public final class SimpleSmtpServer implements AutoCloseable {
    ...
    private SimpleSmtpServer(ServerSocket serverSocket) {
        ...
        this.workerThread = new Thread(() -> performWork());
        ...
        try {
            ...
            while (!stopped) {
                ...
                try (Socket socket = serverSocket.accept();
                        Scanner input = new Scanner(new InputStreamReader(socket.getInputStream(), StandardCharsets.UTF_8)).useDelimiter(CRLF);
    ...
    /**
     * Get somebody's mailbox.
     * 
     * @param email         email address, in form of "userid@domain"
     */
    public List<SmtpMessage> getReceivedEmails(String email) {
        return receivedMail.stream().filter(toFilter(email)).collect(Collectors.toList());
    }

    private Predicate<SmtpMessage> toFilter(String email) {
        return t -> {
            List<String> tos = t.getHeaderValues("To");
            if (tos == null)
                return false;
            return tos.stream().anyMatch(emailAddrMatcher(email));
        };
    }

    private Predicate<? super String> emailAddrMatcher(String email) {
        return t -> {
            String address = StringUtils.substringBetween(t, "<", ">");
            return email.equals(address) || email.equals(t);
        };
    }
}
